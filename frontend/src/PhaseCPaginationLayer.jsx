...
function themeFromPage() {
  ...
  return dark
    ? { text: "#F2F1FB", muted: "rgba(242,241,251,.62)", surface: "rgba(255,255,255,.055)", border: "rgba(255,255,255,.10)" }
    : { text: "#14121F", muted: "rgba(20,18,31,.62)", surface: "rgba(255,255,255,.75)", border: "rgba(20,18,31,.08)" };
}
...
function ExtraPostCard({ post, t }) {
  ...
  return <article data-cm-paged-post={post._id} style={{ border: `1px solid ${t.border}`, borderRadius: 20, padding: 16, marginBottom: 16, background: t.surface, backdropFilter: "blur(16px)" }}>
...
export default function PhaseCPaginationLayer() {
  ...
  return createPortal(<div data-cm-pagination style={{ marginTop: 14, paddingBottom: 6 }}>
    {items.map((p) => <ExtraPostCard key={p._id} post={p} t={t} />)}
    {busy && page > 1 && <><SkeletonCard t={t} /><SkeletonCard t={t} /></>}
    {!busy && hasMore && <button onClick={loadMore} style={{ width: "100%", border: "none", borderRadius: 14, padding: "11px 16px", background: "linear-gradient(135deg,#6D5DF6,#A855F7)", color: "white", fontWeight: 700, cursor: "pointer" }}>Load more posts</button>}
