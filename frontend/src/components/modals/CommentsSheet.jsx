...
export default function CommentsSheet({ t, post, profile, onClose, onAddComment, Avatar, byId }) {
  ...
  return (
    <div className="cm-sheet-overlay" style={{ position: "fixed", inset: 0, zIndex: 90, display: "flex", alignItems: "flex-end", justifyContent: "center" }}>
      <div onClick={onClose} style={{ position: "absolute", inset: 0, background: "rgba(6,6,14,.6)", backdropFilter: "blur(4px)" }} />
      <div className="cm-sheet-panel cm-comments-sheet" onClick={(e) => e.stopPropagation()} style={{ position: "relative", width: "100%", maxWidth: 480, maxHeight: "78vh", background: t.bg2, borderRadius: "22px 22px 0 0", border: `1px solid ${t.border}`, borderBottom: "none", display: "flex", flexDirection: "column" }}>
