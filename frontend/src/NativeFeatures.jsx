...
import { Bookmark, ChevronLeft, ChevronRight, Film, Heart, Image as ImageIcon, Loader2, LogOut, Pencil, Play, RefreshCw, Send, Shield, Trash2, Volume2, VolumeX, X } from "lucide-react";
...
export function NativeStories({ me }) {
  ...
  const refresh = async () => { try { setStories((await api.fetchStories()).filter(s => !s.expiresAt || new Date(s.expiresAt) > new Date())); setError(""); } catch (e) { setError(errorText(e)); } };
...
export function NativeReels({ t }) {
  ...
  const load = async () => { setLoading(true); try { const [items, user] = await Promise.all([api.fetchReels(), api.fetchMe()]); setReels(items); setMe(user); setError(""); } catch (e) { setError(errorText(e)); } finally { setLoading(false); } };
...
export function NativeMessages({ onClose }) {
  const [me, setMe] = useState(null), [matches, setMatches] = useState([]), [selected, setSelected] = useState(null), [messages, setMessages] = useState([]), [text, setText] = useState(""), [error, setError] = useState(""), end = useRef(null);
  const load = async () => { try { const [user, list] = await Promise.all([api.fetchMe(), api.fetchMatches()]); setMe(user); setMatches(list); setSelected(s => s || list[0] || null); setError(""); } catch (e) { setError(errorText(e)); } };
  useEffect(() => { load(); }, []);
  useEffect(() => { if (!selected?._id) return; api.fetchMessages(selected._id).then(setMessages).catch(e => setError(errorText(e))); const socket = getChatSocket(); if (!socket) return; socket.emit("chat:join", selected._id); socket.emit("chat:read", { matchId: selected._id }); const incoming = m => { if (String(m.match) === String(selected._id)) setMessages(p => p.some(x => x._id === m._id) ? p : [...p, m]); }; socket.on("chat:message", incoming); return () => { socket.emit("chat:leave", selected._id); socket.off("chat:message", incoming); }; }, [selected?._id]);
  useEffect(() => end.current?.scrollIntoView({ behavior: "smooth" }), [messages.length]);
  const send = async e => { e.preventDefault(); const value = text.trim(); if (!value || !selected) return; setText(""); try { const msg = await api.sendMessage(selected._id, value); setMessages(p => p.some(x => x._id === msg._id) ? p : [...p, msg]); getChatSocket()?.emit("chat:message:delivered", { matchId: selected._id, messageId: msg._id }); } catch (err) { setText(value); setError(errorText(err)); } };
  return <section className={`cm-native-chat ${selected ? "has-selection" : ""}`}><aside><div className="cm-feature-toolbar"><h2>Messages</h2><button onClick={load} style={button}><RefreshCw size={16}/></button><button onClick={onClose} style={button}><X size={17}/></button></div>{matches.map(m => <button className={selected?._id === m._id ? "active" : ""} key={m._id} onClick={() => setSelected(m)}><Avatar user={m.user}/><span><strong>{m.user?.name || "CampusMate user"}</strong><small>{m.user?.collegeName || m.user?.branch || "Matched student"}</small></span></button>)}{!matches.length && <div className="cm-empty">No matches yet.</div>}</aside><main>{selected ? <><header>{selected.user?.name || "Conversation"}<button className="cm-chat-back" onClick={() => setSelected(null)}>Back</button></header><div className="cm-message-list">{error && <div className="cm-inline-error">{error}</div>}{messages.map(m => <div key={m._id} className={String(m.sender?._id || m.sender) === String(me?._id) ? "mine" : "theirs"}>{m.text}</div>)}<i ref={end}/></div><form onSubmit={send}><input value={text} onChange={e => setText(e.target.value)} maxLength={2000} placeholder="Message…"/><button aria-label="Send"><Send size={18}/></button></form></> : <div className="cm-empty">Choose a conversation</div>}</main></section>;
}
...
export function NativeProfile({ me, onUserChange, onLogout }) {
  ...
  const save = async () => { try { const { privacy, ...values } = form; const user = await api.updateUserProfile(me._id, { ...values, privacy: { ...(me.privacy || {}), postsDefault: privacy }, interests: form.interests.split(",").map(x => x.trim()).filter(Boolean) }); onUserChange(user); setEdit(false); } catch (e) { setError(errorText(e)); } };
...
function MediaGrid({ items, reels }) { return !items.length ? <div className="cm-empty">No content yet.</div> : <div className="cm-profile-grid">{items.map(x => { const media = api.resolveMediaUrl(x.videoUrl || x.thumbnailUrl || x.media?.[0]?.url); return <div key={x._id}>{media ? <img src={media} alt=""/> : reels ? <Film/> : <ImageIcon/>}</div>; })}</div>; }
