...
export function NativeMessages({ onClose }) {
  const [me, setMe] = useState(null), [matches, setMatches] = useState([]), [selected, setSelected] = useState(null), [messages, setMessages] = useState([]), [text, setText] = useState(""), [file, setFile] = useState(null), [showEmoji, setShowEmoji] = useState(false), [onlineIds, setOnlineIds] = useState(() => new Set()), [sending, setSending] = useState(false), [loadingMatches, setLoadingMatches] = useState(true), [loadingMessages, setLoadingMessages] = useState(false), [listError, setListError] = useState(""), [messageError, setMessageError] = useState(""), [messageAttempt, setMessageAttempt] = useState(0), [query, setQuery] = useState(""), [conversationFilter, setConversationFilter] = useState("all"), [connected, setConnected] = useState(false), [peerTyping, setPeerTyping] = useState(false), end = useRef(null), fileInput = useRef(null), typingTimer = useRef(null);
  ...
  const visibleMatches = matches.filter(match => {
    const matchesQuery = (match.user?.name || "").toLowerCase().includes(query.trim().toLowerCase());
    if (!matchesQuery) return false;
    if (conversationFilter === "unread") return Number(match.unreadCount || 0) > 0;
    if (conversationFilter === "groups") return Boolean(match.isGroup);
    return true;
  });
  const conversationTime = value => value ? new Date(value).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "";
  return <section className={`cm-native-chat ${selected ? "has-selection" : ""}`}>
    <aside><div className="cm-chat-list-head"><div><h2>Messages</h2><span className={connected ? "connected" : ""}>{connected ? <Wifi size={12}/> : <WifiOff size={12}/>} {connected ? "Live" : "Reconnecting"}</span></div><div><button onClick={load} aria-label="Refresh conversations"><RefreshCw size={16}/></button><button onClick={onClose} aria-label="Close messages"><X size={17}/></button></div></div><label className="cm-chat-search"><Search size={16}/><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search conversations"/></label><div className="cm-chat-filters" role="tablist" aria-label="Conversation filters">{["all","unread","groups"].map(item => <button type="button" role="tab" aria-selected={conversationFilter === item} className={conversationFilter === item ? "active" : ""} key={item} onClick={() => setConversationFilter(item)}>{item[0].toUpperCase() + item.slice(1)}</button>)}</div>{listError && <div className="cm-chat-state error"><span>{listError}</span><button onClick={load}>Try again</button></div>}{loadingMatches ? <FeatureLoading/> : visibleMatches.map(m => <button className={selected?._id === m._id ? "active" : ""} key={m._id} onClick={() => setSelected(m)}><span className="cm-chat-avatar"><Avatar user={m.user}/>{onlineIds.has(String(m.user?._id)) && <i/>}</span><span><strong>{m.user?.name || "CampusMate user"}</strong><small>{onlineIds.has(String(m.user?._id)) ? "Online now" : lastSeenText(m.user?.lastActiveAt)}</small></span><span className="cm-chat-meta">{conversationTime(m.lastMessageAt)}{Number(m.unreadCount || 0) > 0 && <b>{m.unreadCount}</b>}</span><ChevronRight size={16}/></button>)}{!loadingMatches && !listError && !visibleMatches.length && <div className="cm-chat-state"><MessageEmptyIcon/><strong>{query ? "No conversation found" : conversationFilter === "all" ? "No matches yet" : `No ${conversationFilter} conversations`}</strong><span>{query ? "Try another name." : conversationFilter === "all" ? "Match with a student to start chatting." : "Nothing needs your attention here."}</span></div>}</aside>
