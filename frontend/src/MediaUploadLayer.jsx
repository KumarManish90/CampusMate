    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      ...
      <div style={{ position: "relative", width: "100%", maxWidth: 520, maxHeight: "90vh", overflowY: "auto", borderRadius: 24, background: "#10142a", border: "1px solid rgba(255,255,255,.12)", padding: 20, boxShadow: "0 30px 80px -20px rgba(0,0,0,.6)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          <div style={{ width: 38, height: 38, borderRadius: 11, background: "rgba(109,93,246,.15)", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Icon size={18} color="#8b7cff" />
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ color: "#f2f1fb", fontWeight: 700, fontSize: 16 }}>{meta.title}</div>
            <div style={{ color: "rgba(242,241,251,.55)", fontSize: 11.5 }}>Real Cloudinary upload</div>
          </div>
          <button onClick={close} disabled={busy} style={{ width: 36, height: 36, borderRadius: 10, border: "1px solid rgba(255,255,255,.1)", background: "transparent", color: "rgba(255,255,255,.7)", cursor: busy ? "not-allowed" : "pointer" }}>
            <X size={17} />
          </button>
        </div>
            ...
            {objectUrls.map(({ file, url }) => (
              <div key={`${file.name}-${file.lastModified}`} style={{ position: "relative", borderRadius: 14, overflow: "hidden", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.1)" }}>
                {file.type.startsWith("video/") ? (
                  <video src={url} controls muted playsInline style={{ width: "100%", maxHeight: 280, display: "block", objectFit: "cover" }} />
                ) : (
                  <img src={url} alt={file.name} style={{ width: "100%", maxHeight: 280, display: "block", objectFit: "cover" }} />
                )}
                <div style={{ position: "absolute", left: 8, right: 8, bottom: 8, padding: "5px 7px", borderRadius: 8, background: "rgba(0,0,0,.55)", color: "#fff", fontSize: 10.5 }}>
